...
static bool minmea_scan(const char* sentence, const char* format, ...) {
    ...
    while(*format) {
        ...
        case 't': { // time
            ...
            if(field_len >= 6) {
                char tmp[3] = {0};
                tmp[0] = field[0]; tmp[1] = field[1];
                t->hours = atoi(tmp);
                tmp[0] = field[2]; tmp[1] = field[3];
                t->minutes = atoi(tmp);
                tmp[0] = field[4]; tmp[1] = field[5];
                t->seconds = atoi(tmp);
                if(field_len > 7 && field[6] == '.') {
                    // parse sub-seconds as microseconds
                    int32_t us = 0;
        ...
        case 's': { // string (pointer into sentence — copies up to 16 chars)
...
enum minmea_sentence_id minmea_sentence_id(const char* sentence, bool strict) {
    ...
    // Try to match after talker id (GP, GL, GN, GA, etc.)
    // Sentence id starts at position 3 if 2-char talker id
    const char* id = p + 2; // skip 2-char talker id
...
bool minmea_parse_rmc(struct minmea_sentence_rmc* frame, const char* sentence) {
    // $GPRMC,time,status,lat,N,lon,E,speed,course,date,variation,dir,mode*cs
    ...
    return minmea_scan(
        ...
        (frame->valid = (status == 'A'),
         (lat_dir == 'S') ? (frame->latitude.value = -frame->latitude.value) : 0,
         (lon_dir == 'W') ? (frame->longitude.value = -frame->longitude.value) : 0,
         true);
}

bool minmea_parse_gga(struct minmea_sentence_gga* frame, const char* sentence) {
    // $GPGGA,time,lat,N,lon,E,fix,sats,hdop,alt,M,height,M,age,ref*cs
...
float minmea_rescale(const struct minmea_float* f, int32_t new_scale) {
    if(f->scale == 0) return 0.0f;
    return (float)f->value * (float)new_scale / (float)f->scale;
}
